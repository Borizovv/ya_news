...
class TestCommentCreation(TestCase):
    # Текст комментария понадобится в несколькихз местах кода,
    ...
    def test_user_can_create_comment(self):
        # Совершаем запрос через авторизированный клиент
        response = self.auth_client.post(self.url, data=self.form_data)
        # Проверяем, что редирект привел к разделу с комментариями.
        # Используем якорь, для прокрутки страницы до раздела с комментариями
        # id="comments"
...
class TestCommentEditDelete(TestCase):
    ...
    # (в отличии от объектов БД), им не нужны ссылки на self или cls,
    ...
    def setUpTestData(cls) -> None:
        ...
        new_url = reverse('news:detail', args=(
            cls.news.id,))  # Адрес новости
        # Адрес блока с комментариями
        cls.url_to_comments = new_url + '#comments'
